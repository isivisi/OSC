...
class uiAudioDevice(tk.Frame):
    ...
    def setup(self):
        ...
        # visual volume stuff
        self.volume = tk.Canvas(self, width=50, height=300)
        self.volume.grid(row=1, column=0)
        self.volume.create_rectangle(2, 0, 26, 300, fill="#d3d3d3")
        self.volume.create_rectangle(26, 0, 50, 300, fill="#d3d3d3")
        self.leftChannel = self.volume.create_rectangle(2, 200, 26, 300, fill="#66ff00")
        self.rightChannel = self.volume.create_rectangle(26, 200, 50, 300, fill="#66ff00")
        threading.Thread(target=self.getVolume).start()

        self.volumeScale = tk.Scale(self, from_=100, to=0, orient="vertical").grid(row=1, column=1, sticky="w")

        self.toggle = tk.Button(self, text="Enable/Disable", fg="black", command=self.toggleDevice)
        self.toggle.grid(row=2, columnspan=2)
    ...
    def getVolume(self):
        if (self.audioDevice != None):
            ...
            self.leftChannel = self.volume.create_rectangle(avg + 2, avg, 26, 300, fill="#66ff00")
            self.rightChannel = self.volume.create_rectangle(avg + 26, avg, 50, 300, fill="#66ff00")
